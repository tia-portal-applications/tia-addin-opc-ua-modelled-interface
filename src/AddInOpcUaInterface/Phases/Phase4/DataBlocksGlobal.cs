...
{
    internal static class DataBlocksGlobal
    {
        private static int _numberOfProcessedDBs = 0;

        /// <summary>
        /// Resets the internal count of processed datablock elements to zero.
        /// </summary>
        ...
        {
            string parentFolder = "DataBlocksGlobal";
            foreach (PlcBlock block in blockGroup.Blocks)
            {
                //Only process Datablocks
                if (block.ProgrammingLanguage == ProgrammingLanguage.DB && AddInExecutionContext.Current.GlobalDBsAccessLevel != 0)
                {
                    NewDataBlockElement(block, parentFolder, false);
                }
                //Only process failsafe Datablocks
                if (block.ProgrammingLanguage == ProgrammingLanguage.F_DB && AddInExecutionContext.Current.SafetyGlobalDBsAccessLevel != 0)
                {
        ...
        /// <summary>
        /// Creates an UAObject to represent the Global DB in the server interface.
        /// </summary>
        ...
        private static void NewDataBlockElement(PlcBlock block, string parentFolder, bool isSafety)
        {
            ...
            {
                DisplayMessage.GetExclusiveAccess().Text = $@"Adding ""Global"" DBs to the server interface... Count: {_numberOfProcessedDBs}";
            }
            ...
            {
                DisplayMessage.ErrorMessage($@"Unexpected error ocurred while exporting the .txt file of the {block.Name} Global DB.");
            }
            #endregion
            ...
            {
                DisplayMessage.ErrorMessage($@"Unexpected error ocurred while importing the .txt file of the {block.Name} Global DB.");
            }
            #endregion
            ...
            XElement attributeList = new XElement("Empty");
            string dbAccessibleFromOPCUA = "false";
            string typeOfDB = string.Empty;

            if (inputElement.Element("SW.Blocks.GlobalDB") != null)
            {
                attributeList = inputElement.Element("SW.Blocks.GlobalDB").Element("AttributeList");
                typeOfDB = "Global DB";

                // Check if the Global DB is accessible via OPC UA
            ...
            #region  CONVERT THE DATA BLOCK GLOBAL VARIABLES TO OPC UA FORMAT
            ...
            if (currentElementsCount == newElementsCount && AddInExecutionContext.Current.KeepEmptyDBs == false) // No new nodes were added to the server interface
            {
                ...
                LogMessages.PublishLog($@"EMPTY DB: The Global DB ""{blockName}"" has not been added to the server interface as it does not contain any variables accesible via OPC UA.");
            }
            #endregion
        }
    }
